...
export default function MarkdownViewer({ markdown, repoName }: MarkdownViewerProps) {
  const htmlContent = useMemo(() => {
    ...
    // 2. Fix relative images if repoName is provided
    if (repoName) {
      processed = processed.replace(
        /!\[(.*?)\]\(((?!https?:\/\/|data:|\/\/)[^)]+)\)/g,
        (_, alt, path) => {
    ...
    // Custom image renderer with badge detection
    renderer.image = ({ href, title, text }) => {
      ...
      return `<img src="${href}" alt="${text || "image"}" class="md-img"${titleAttr} loading="lazy" />`;
    };
